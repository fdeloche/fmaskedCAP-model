...
class RBFNet(nn.Module):
	...
	def __init__(self, n, f_min=800, f_max=15000, sig=0.1, mult_factor=1., init_random=False):
		'''
		...
		:param mult_factor: multiplication factor for output'''
	...
	def create_from_jsonfile(cls, filename):
		...
		with open(filename) as json_file:
			...
			mult_factor=float(data['mult_factor'])
		return cls(n, f_min=f_min, f_max=f_max, sig=sig, mult_factor=mult_factor) #nb:init random always False
